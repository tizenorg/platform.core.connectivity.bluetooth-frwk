					...
					GArray *in_param5)
{
	...
	if (ret_val != CYNARA_API_SUCCESS) {
		BT_ERR("Fail to get default client method: %d", ret_val);
		return FALSE;
	}

	ret_val = cynara_creds_get_default_user_method(&user_creds_method);
	if (ret_val != CYNARA_API_SUCCESS) {
		BT_ERR("Fail to get default user method: %d", ret_val);
		return FALSE;
	}
#endif
	...
	if (ret_val != CYNARA_API_SUCCESS) {
		BT_ERR("Fail to get user credential: %d", ret_val);
		if (client_creds)
			free(client_creds);
		return FALSE;
	}
...
#if 0
		char err_msg[256] = {0, };
		BT_ERR("%s", cynara_strerror(result, err_msg, sizeof(err_msg)));
#endif
