...
#include "uts_bluetooth_check_adapter.h"
...
int enable_thread_callback(void)
{
	int 	return_3;
	printf("thread_callback\n");
	sleep(4);
	printf("1\n");
	return_3=bluetooth_check_adapter();
	printf("2\n");
	if(	return_3!=BLUETOOTH_ADAPTER_ENABLED)
	{
...
int disable_thread_callback(void)
{
	int 	return_3;
	printf("thread_callback\n");
	sleep(4);
	...
	return_3=bluetooth_check_adapter();
	printf("2\n");
	if(	return_3!=BLUETOOTH_ADAPTER_DISABLED)
	{
	tet_printf("Api failed");
	tet_result(TET_FAIL);
	}
...
void utc_bluetooth_check_adapter_1(void)
{
	...
	printf("2\n");
	if(	return_3 < BLUETOOTH_ADAPTER_DISABLED || return_3 > BLUETOOTH_ADAPTER_CHANGING_DISABLE)
	{
	tet_printf("Api failed");
	tet_result(TET_FAIL);
	}
...
/*
	...
	if (ret == 0)
	{
	    ...
	    if(ret == 0)
	    {
		     ret = pthread_create(&thread_id, &tattr,
			 					enable_thread_callback, NULL);
		     if (ret != 0)
		     {
...
void utc_bluetooth_check_adapter_2(void)
{
	int return_3;
	int ret_val;
	int ret=0;
