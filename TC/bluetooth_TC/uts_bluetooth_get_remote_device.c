...
#include "uts_bluetooth_get_remote_device.h"
...
// Not support API
#if 0
void utc_bluetooth_get_remote_device_1(void)
{
	...
	return_16=bluetooth_get_remote_device(&device_address);
	if(	return_16<0)
	{
...
void utc_bluetooth_get_remote_device_1(void)
{
	const bluetooth_device_address_t device_address={{0x00,0x80,0x98,0xE7,0x34,0x82}};
	int return_16;
	int ret_val;
	ret_val = bluetooth_register_callback(bt_event_callback, NULL);
	...
	if(	return_16<0)
	{
	tet_printf("Api failed: %d", return_16);
	tet_result(TET_FAIL);
	}
	else
	{
...
void utc_bluetooth_get_remote_device_2(void)
{
	bluetooth_device_address_t* device_address;
	int return_16;
	int ret_val;
