...
#include "uts_bluetooth_is_discovering.h"
...
int thread_callback(void)
{
	...
	sleep(1);
	return_11=bluetooth_is_discovering();
	if(	return_11==0)
	{
	...
	// let the discovery stop before quit so that when utc_bluetooth_is_discovering_2 is called discovery should be off
...
void utc_bluetooth_is_discovering_1(void)
{
	int return_11;
	int ret_val;
...
void utc_bluetooth_is_discovering_2(void)
{
	...
	classOfDeviceMask =0;
	return_11=bluetooth_start_discovery(max_response,discovery_duration,classOfDeviceMask);
