...
#include "uts_bluetooth_cancel_discovery.h"
...
int thread_callback(void)
{
	...
	sleep(1);
	return_10=bluetooth_cancel_discovery();
	if(	return_10<0)
	{
...
void utc_bluetooth_cancel_discovery_1(void)
{
	...
	// Stop discovery if BT is in discovering.
	bluetooth_cancel_discovery();

	max_response =0;
	discovery_duration =0;
	classOfDeviceMask =0;
	ret_val=bluetooth_start_discovery(max_response,discovery_duration,classOfDeviceMask);
...
void utc_bluetooth_cancel_discovery_2(void)
{
	...
	if(	return_10 >= 0)
	{
	tet_printf("Api failed: %d", return_10);
	tet_result(TET_FAIL);
	}
	else
	{
