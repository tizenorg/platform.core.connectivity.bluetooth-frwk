...
#include "uts_bluetooth_unbond_device.h"
...
void bt_event_callback(int event, bluetooth_event_param_t* param, void *user_data)
{
	...
	switch(event)
	{
		...
		case BLUETOOTH_EVENT_BONDING_FINISHED:
		{
			...
			return_14=bluetooth_unbond_device(&device_info->device_address);
			if( return_14<0)
			{
				tet_printf("Api failed: %d", return_14);
				tet_result(TET_FAIL);
			}
			else
			{
...
void utc_bluetooth_unbond_device_1(void)
{
	bluetooth_device_address_t device_address={{0x00,0x02,0x78,0xDE,0x48,0x8F}}; // Samsung Stereo Headset
	...
	return_14=bluetooth_bond_device(&device_address);
	if(	return_14<0)
	{
		tet_printf("bond failed: %d", return_14);
		tet_result(TET_FAIL);
	}
	else
	{
		tet_printf("bonding is in progress");
...
void utc_bluetooth_unbond_device_2(void)
{
	...
	device_address=NULL;
	return_14=bluetooth_unbond_device(device_address);
	if(	return_14>=0)
	{
	tet_printf("Api failed: %d", return_14);
	tet_result(TET_FAIL);
