				...
				const char *key, GVariant *val)
{
	...
	g_dbus_connection_emit_signal(conn, NULL,
			...
			"PropertiesChanged",
			g_variant_new("(sa{sv})",MEDIA_PLAYER_INTERFACE,builder),
			&error);
			...
			media_player_settings_t *properties)
{
	...
	case LOOPSTATUS:
		...
		DBG("LOOPSTATUS loop = %s", loopstatus_settings[value].property);
			...
			media_metadata_attributes_t *meta_data)
{
	...
	if (meta_data->artist) {
		...
		val_array = g_variant_new_array(G_VARIANT_TYPE_STRING,
							str_array, 1);
		if (!bluez_set_metadata(adapter, "xesam:artist", val_array)){
			DBG("Error sending the PropertyChanged signal \n");
			return BT_ERROR_OPERATION_FAILED;
		}
	}
	...
	if (meta_data->genre) {
		...
		val_array = g_variant_new_array(G_VARIANT_TYPE_STRING,
							str_array, 1);
		if (!bluez_set_metadata(adapter, "xesam:genre", val_array)){
			DBG("Error sending the PropertyChanged signal \n");
			return BT_ERROR_OPERATION_FAILED;
		}
	}
	...
	if (0 != meta_data->tracknumber){
		...
		if (!bluez_set_metadata(adapter, "xesam:trackNumber", val)){
			DBG("Error sending the PropertyChanged signal \n");
			return BT_ERROR_OPERATION_FAILED;
		}
	}
...
void bt_register_media_player(struct _bluez_adapter * adapter)
{
	...
	g_dbus_proxy_call(adapter->media_proxy,
			"RegisterPlayer",
			g_variant_new("(oa{sv})", BT_MEDIA_OBJECT_PATH, builder),
			0, -1, NULL,
