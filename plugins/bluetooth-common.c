...
static int bt_pairing_agent_on(bundle* b)
{
	...
	DBG("create notification for '%s' event", event_type);
	...
	if (!g_strcmp0(event_type, "RequestPinCode")) {
		...
		// Use notification_text_type field to handle different notifications types :
		// NOTIFICATION_TEXT_TYPE_INFO_1 : confirmation popup ('yes' and 'no' buttons)
		// NOTIFICATION_TEXT_TYPE_INFO_2 : display popup (just an 'OK' button)
		// NOTIFICATION_TEXT_TYPE_INFO_3 : popup with an user entry (i.e. for pin code)
  ...
  g_free(body);
	...
	if (notification_set_image(noti, NOTIFICATION_IMAGE_TYPE_ICON, BLUETOOTH_ICON_PATH) != NOTIFICATION_ERROR_NONE ) {
		ERROR("Unable to set notification icon path: %s", error_to_string(err));
		return 1;
	}
