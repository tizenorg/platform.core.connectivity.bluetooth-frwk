...
static int hcdfile_fd = -1;
static int termios_baudrate;
static int bdaddr_flag;
static int enable_lpm;
static int enable_hci;
static int use_baudrate_for_download;
static int debug;
static int scopcm;
static int i2s;
static int no2bytes;
...
parse_cmd_line(int argc, char **argv)
{
	...
	if (optind < argc) {
		...
		uart_fd = open(argv[optind], O_RDWR | O_NOCTTY);
		if (uart_fd == -1) {
...
#else
void init_brcm_bluetooth(int argc, char **argv)
{
#endif
	printf("before parse_cmd\n");
	...
	printf("before init_uart\n");

	init_uart();

	printf("before parse_reset\n");
	proc_reset();

	printf("after parse_reset\n");
	if (use_baudrate_for_download) {
		if (termios_baudrate) {
			printf("before proc_baudrate1\n");
			proc_baudrate();
		}
	}

	printf("before proc_patchram\n");
	if (hcdfile_fd > 0) {
		printf("before proc_patchram1\n");
		proc_patchram();
	}

	printf("before parse_baudrate\n");

	if (termios_baudrate) {
		printf("before parse_bdaddr1\n");
		proc_baudrate();
	}

	printf("before parse_bdaddr\n");
	...
	printf("before parse_lpm\n");

	if (enable_lpm) {
		printf("before parse_lpm1\n");
		proc_enable_lpm();
	}

	printf("before parse_sco\n");

	if (scopcm) {
		printf("before parse_sco1\n");
		proc_scopcm();
	}

	printf("before parse_i2s\n");
	if (i2s) {
		printf("before parse_i2s1\n");
		proc_i2s();
	}

	if (enable_hci) {
		printf("before enable_hci\n");
		proc_enable_hci();
...
void deinit_brcm_patchram(void)
{
	close(uart_fd);
	uart_fd = -1;
}
